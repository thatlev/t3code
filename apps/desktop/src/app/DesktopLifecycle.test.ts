...
function makeDesktopWindowLayer(
  ...
  return Layer.succeed(DesktopWindow.DesktopWindow, {
    ...
    // Ours: torn-off chat windows. Upstream hoisted this mock into a shared
    // factory that predates the detached-window API, so the member has to be
    // reinstated here or the layer no longer satisfies the service.
    openDetachedWindow: () => Effect.die("unexpected window creation"),
