...
import {
  AGENT_MICRO_CONTROL_CHANNEL,
  AGENT_MICRO_STATE_CHANNEL,
  AgentMicroCommandDecoder,
  encodeReports,
} from "./protocol";

describe("AgentMicro BLE framing", () => {
  it("round-trips fragmented newline-delimited phone commands", () => {
    ...
    const reports = encodeReports(AGENT_MICRO_CONTROL_CHANNEL, `${JSON.stringify(command)}\n`);
    const decoder = new AgentMicroCommandDecoder();
  ...
  it("frames host state as fixed-size channel-three reports", () => {
    const reports = encodeReports(AGENT_MICRO_STATE_CHANNEL, {
    ...
    expect(reports.every((report) => report[0] === AGENT_MICRO_STATE_CHANNEL)).toBe(true);
  ...
  it("terminates every frame with a newline so a dropped fragment costs one frame", () => {
    ...
    const reports = encodeReports(AGENT_MICRO_STATE_CHANNEL, { type: "workspace-state" });
  ...
  it("ignores unrelated and malformed reports without poisoning the next command", () => {
    const decoder = new AgentMicroCommandDecoder();
    const malformed = new Uint8Array([AGENT_MICRO_CONTROL_CHANNEL, 62]);
